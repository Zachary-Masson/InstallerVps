...
nodeJSInstall() {
    ...
    echo "$(date '+%d/%m/%Y') | $(date '+%H:%M') > [IV] Installation de NodeJs V16 ! | user => ($USER)" >> /home/InstallerVps/logs.txt
    npm i pm2 -g
    echo "$(date '+%d/%m/%Y') | $(date '+%H:%M') > [NPM] Installation de 'PM2' ! | user => ($USER)" >> /home/InstallerVps/logs.txt
    sleep 3
    npm i --save
    echo "$(date '+%d/%m/%Y') | $(date '+%H:%M') > [NPM] Installe les dépendances du panel ! | user => ($USER)" >> /home/InstallerVps/logs.txt
    pm2 start main.js --name IV_panel
    echo "$(date '+%d/%m/%Y') | $(date '+%H:%M') > [PM2] Lancement de l'application LINK : http://$(ip -4 addr show eth0 | grep -oP "(?<=inet ).*(?=/)"):7856 ! | user => ($USER)" >> /home/InstallerVps/logs.txt
}
...
debian() {
    ...
    echo "$(date '+%d/%m/%Y') | $(date '+%H:%M') > [IV] Dossier [IV_dataSave] créer par le programme ($USER)!" >> /home/InstallerVps/logs.txt
    mkdir /home/InstallerVps/IV_tempData
    echo "$(date '+%d/%m/%Y') | $(date '+%H:%M') > [IV] Dossier [IV_tempData] créer par le programme ($USER)!" >> /home/InstallerVps/logs.txt
